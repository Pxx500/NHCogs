...
class BotProxyPublisherTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bot_message_edit_and_delete_use_tracked_revisions(self) -> None:
        message = SimpleNamespace(edit=mock.AsyncMock(), delete=mock.AsyncMock())
        channel = SimpleNamespace(fetch_message=mock.AsyncMock(return_value=message))
        record = SimpleNamespace(
            guild_id=100,
            channel_id=200,
            message_id=300,
            sender=ProxySender.BOT,
            webhook_id=None,
            content="Before",
            revision=1,
        )
        edited = SimpleNamespace(revision=2)
        deleted = SimpleNamespace(revision=3)
        store = SimpleNamespace(
            claim_message_transition=mock.AsyncMock(side_effect=["edit", "delete"]),
            release_message_transition=mock.AsyncMock(),
            edit_message=mock.AsyncMock(return_value=edited),
            mark_message_deleted=mock.AsyncMock(return_value=deleted),
        )
        publisher = bot_proxy.BotProxyPublisher(store)

        with mock.patch.dict(sys.modules, {"discord": discord_module()}):
            self.assertIs(
                await publisher.edit_tracked(
                    record=record,
                    channel=channel,
                    content="After",
                    moderator_id=400,
            ...
            record.revision = 2
            self.assertIs(
                await publisher.delete_tracked(
                    record=record,
                    channel=channel,
                    moderator_id=400,
                ),
        ...
        store.edit_message.assert_awaited_once_with(
            guild_id=100,
            channel_id=200,
            message_id=300,
            expected_revision=1,
            content="After",
            moderator_id=400,
            transition_token="edit",
        )

    async def test_character_edit_and_delete_use_owned_webhook(self) -> None:
        webhook = SimpleNamespace(
            id=500,
            edit_message=mock.AsyncMock(),
            delete_message=mock.AsyncMock(),
        )
        parent = SimpleNamespace(webhooks=mock.AsyncMock(return_value=[webhook]))
        ...
        store = SimpleNamespace(
            claim_message_transition=mock.AsyncMock(side_effect=["edit", "delete"]),
            release_message_transition=mock.AsyncMock(),
            edit_message=mock.AsyncMock(return_value=SimpleNamespace(revision=2)),
            mark_message_deleted=mock.AsyncMock(
                return_value=SimpleNamespace(revision=3)
            ),
        )
        publisher = bot_proxy.BotProxyPublisher(store)
        ...
        webhook.delete_message.assert_awaited_once()

    async def test_stale_tracked_actions_are_rejected_before_discord_mutation(self) -> None:
        message = SimpleNamespace(edit=mock.AsyncMock(), delete=mock.AsyncMock())
        channel = SimpleNamespace(fetch_message=mock.AsyncMock(return_value=message))
        ...
        store = SimpleNamespace(
            claim_message_transition=mock.AsyncMock(
                side_effect=bot_proxy_store.StaleMessageRevision(300)
            ),
            release_message_transition=mock.AsyncMock(),
        )
        publisher = bot_proxy.BotProxyPublisher(store)
        ...
        for action in ("edit", "delete"):
            with self.subTest(action=action), self.assertRaises(
                bot_proxy_store.StaleMessageRevision
            ):
                if action == "edit":
                    await publisher.edit_tracked(
                        record=stale,
    ...
    async def test_separate_publishers_cannot_mutate_one_revision_concurrently(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            store = bot_proxy_store.BotProxyStore(Path(tempdir) / "bot_proxy.sqlite")
            await store.initialize()
            record = await store.record_message(
                guild_id=100,
                channel_id=200,
                message_id=300,
                moderator_id=400,
            ...
            first = bot_proxy.BotProxyPublisher(store)
            second = bot_proxy.BotProxyPublisher(store)
            ...
            current = await store.get_message(100, 200, 300)
            self.assertFalse(isinstance(first_result, Exception))
            self.assertIsInstance(
                second_result,
                bot_proxy_store.StaleMessageRevision,
            )
            self.assertEqual(message.content, "First")
            self.assertEqual(current.content, "First")
