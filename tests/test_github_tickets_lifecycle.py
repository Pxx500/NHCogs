...
class GitHubTicketsLifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_configured_github_runtime_starts_after_discord_restore_and_closes(self):
        with isolated_githubtickets_modules(self.data_path) as modules:
            ...
            cog = modules.githubtickets.GitHubTickets(bot, mock.Mock(report_operational_error=mock.AsyncMock()))
    ...
    async def test_shared_githubtickets_token_update_restarts_only_its_runtime(self):
        with isolated_githubtickets_modules(self.data_path) as modules:
            cog = modules.githubtickets.GitHubTickets(FakeBot(ready=True), mock.Mock(report_operational_error=mock.AsyncMock()))
