...
class Honeypot(Cog):
    ...
    async def _imagescan_profile(self, guild_id: int) -> dict[str, int]:
        async with self._imagescan_db_lock:
            return await asyncio.to_thread(self._imagescan_store.profile, guild_id)

    async def _imagescan_increment_profile(self, guild_id: int, increments: dict[str, int]) -> None:
        async with self._imagescan_db_lock:
            await asyncio.to_thread(
                self._imagescan_store.increment_profile,
                guild_id,
                increments,
            )
    ...
    async def _imagescan_update_sample_file(
        ...
        async with self._imagescan_db_lock:
            await asyncio.to_thread(
                self._imagescan_store.update_file,
                guild_id,
                sample_id,
                file_path,
                file_size,
    ...
    async def _imagescan_delete_sample(self, guild_id: int, sample_id: str) -> None:
        async with self._imagescan_db_lock:
            await asyncio.to_thread(self._imagescan_store.delete, guild_id, sample_id)

    async def _imagescan_deactivate_sample(self, guild_id: int, sample_id: str) -> None:
        async with self._imagescan_db_lock:
            await asyncio.to_thread(
                self._imagescan_store.deactivate,
                guild_id,
                sample_id,
            )
        ...
        try:
            async with self._imagescan_db_lock:
                status = await asyncio.to_thread(
                    self._imagescan_store.publish_file_sample,
                    sample,
                    data,
                    path,
                )
    ...
    async def _imagescan_export_rows(self, guild_id: int) -> list[dict[str, typing.Any]]:
        async with self._imagescan_db_lock:
            return await asyncio.to_thread(self._imagescan_store.export_rows, guild_id)
